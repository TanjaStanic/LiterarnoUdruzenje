...
@Service
@Component
public class SaveNewManuscript implements JavaDelegate {
    @Autowired
    ...
    public void execute(DelegateExecution delegateExecution) throws Exception {
        List<FormFieldDto> fields = (List<FormFieldDto>) delegateExecution.getVariable("Improve manuscript");
        List<FormFieldDto> dtos = (List<FormFieldDto>) delegateExecution.getVariable("Book details form");
        Document d = new Document();
        Book book = new Book();
        for(FormFieldDto f: fields) {
            if(f.getFieldId().equals("NewManuscriptId")) {
